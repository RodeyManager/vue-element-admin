...
export default {
  ...
  methods: {
    // 获取整个菜单列表
    getMenuList() {
      if (this.menuList.length > 0) return new Promise((resolve, reject) => resolve(this.menuList));
      return API.sys.menu.select().then(({ data }) => {
    ...
    init(id) {
      ...
      this.getMenuList().then(() => {
        if (!this.dataForm.id) {
          ...
          // 修改
          API.sys.menu.info(this.dataForm.id).then(({ data }) => {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          var tick = !this.dataForm.id ? API.sys.menu.add(params) : API.sys.menu.update(params);
          tick.then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
