<template>
  <div class="site-content site-content--tabs">
    <el-tabs
      ...
      :closable="$store.state.contentTabs.length > 0"
  ...
  methods: {
    ...
    removeTabHandle(tabName) {
      ...
      if (newTabs.length === 0) {
        ...
        this.UPDATE_MENU_NAV_ACTIVE({ menuId: 0 });
        this.updateMenuActive({ state: this.$store.state, menuId: 0, sidebarCollapse: true });
      }
      // 当前选中tab被删除
    ...
    ...mapActions(['updateMenuActive']),
    ...mapMutations(['UPDATE_CONTENT_TABS', 'UPDATE_CONTENT_TABS_ACTIVE_NAME', 'UPDATE_MENU_NAV_ACTIVE'])
  }
};
</script>
